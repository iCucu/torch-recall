...
查询: `(city == "北京" AND price < 100) OR (gender == "女" AND NOT city == "广州" AND price >= 50)`

这一条查询覆盖了所有核心机制: OR (多 conjunction)、AND (多谓词)、NOT (取反)、数值范围 (<, >=)、离散等值。
...
Or(
  ...
  And(
    Predicate("gender", "==", "女"),
    Not(Predicate("city", "==", "广州")),
    Predicate("price", ">=", 50.0)
  )
)
```
...
已经是 DNF 形式 (OR of ANDs)，无需展开：

```
DNF = [
  conjunction 0: [city=北京, price<100],
  conjunction 1: [gender=女, NOT city=广州, price>=50],
]
```
...
去重后共 3 个 bitmap 谓词 + 2 个数值谓词:

```
bitmap 谓词:
  bp_idx 0 → city=北京,    bitmap_indices[0]=1  (行 1), bitmap_valid[0]=True
  bp_idx 1 → gender=女,    bitmap_indices[1]=3  (行 3), bitmap_valid[1]=True
  bp_idx 2 → city=广州,    bitmap_indices[2]=2  (行 2), bitmap_valid[2]=True
  bp_idx 3..15 → padding,  bitmap_valid=False

数值谓词:
  np_idx 0 → price < 100,  numeric_fields[0]=0, ops[0]=1(LT),  values[0]=100.0, numeric_valid[0]=True
  np_idx 1 → price >= 50,  numeric_fields[1]=0, ops[1]=4(GE),  values[1]=50.0,  numeric_valid[1]=True
  np_idx 2..7 → padding,   numeric_valid=False

negation_mask:
  bp_idx 0 → False   (city=北京 不取反)
  bp_idx 1 → False   (gender=女 不取反)
  bp_idx 2 → True    (city=广州 取反!)
  bp_idx 3..15, np_idx 0..7 → False
```

**Step 2: 填充 conj_matrix** (`_fill_conj_matrix`)

P_TOTAL=24 列 (前 16 列 bitmap 谓词，后 8 列数值谓词)。每格表示「该 conjunction 是否使用该谓词」:

```
                          bp0    bp1    bp2    bp3..15    np0    np1    np2..7
                         (北京)  (女)  (广州)  (padding) (价<100)(价≥50) (padding)
conj 0 (北京 & 价<100):  [ 1,     0,     0,     0..0,      1,     0,     0..0 ]
conj 1 (女&!广州&价≥50): [ 0,     1,     1,     0..0,      0,     1,     0..0 ]
conj 2..7 (padding):     [ 0,     0,     0,     0..0,      0,     0,     0..0 ]
...
bp = bitmaps[bitmap_indices]
  ...
  bp[1] = bitmaps[3] = 0b000110  (gender=女:  item 1,2)
  bp[2] = bitmaps[2] = 0b001000  (city=广州:  item 3)
  bp[3..15] = padding
```
...
np_idx 0 (price < 100):
  [68<100, 128<100, 299<100, 35<100, 15<100, 88<100]
  = [T, F, F, T, T, T]  → 打包 → 0b111001 = {item 0, 3, 4, 5}

np_idx 1 (price >= 50):
  [68≥50, 128≥50, 299≥50, 35≥50, 15≥50, 88≥50]
  = [T, T, T, F, F, T]  → 打包 → 0b100111 = {item 0, 1, 2, 5}
```

合并为 all_bm[0..23]:

```
all_bm[0]  = 0b010101  (city=北京)
all_bm[1]  = 0b000110  (gender=女)
all_bm[2]  = 0b001000  (city=广州)       ← 待取反
all_bm[3..15] = padding
all_bm[16] = 0b111001  (price<100)
all_bm[17] = 0b100111  (price>=50)
all_bm[18..23] = padding
```

#### 阶段 1.5: 取反 (negation)

`negation_mask[2]=True`，对 all_bm[2] 做 `XOR all_ones & valid_mask`:

```
all_bm[2] = (0b001000 XOR 0b111111) & 0b111111
           =  0b110111 & 0b111111
           =  0b110111                    (NOT 广州: item 0,1,2,4,5)
```

其余 negation_mask 为 False，不变。

#### 阶段 2: 有效性掩码

无效谓词 (valid=False) 替换为 all_ones (AND 单位元，不影响结果):

```
all_bm[0]  = 0b010101  ← 有效 (city=北京)
all_bm[1]  = 0b000110  ← 有效 (gender=女)
all_bm[2]  = 0b110111  ← 有效 (NOT city=广州, 已取反)
all_bm[3]  = 0b111111  ← 无效 → all_ones
...
all_bm[15] = 0b111111  ← 无效 → all_ones
all_bm[16] = 0b111001  ← 有效 (price<100)
all_bm[17] = 0b100111  ← 有效 (price>=50)
all_bm[18] = 0b111111  ← 无效 → all_ones
...
...
24 次迭代中，只有 p=0, 1, 2, 16, 17 这 5 次产生有效计算 (conj_matrix 中有 True)。其余 19 次 all_bm=all_ones，AND 后不变，省略。
...
**迭代 p=1** — 谓词: gender=女, `all_bm[1]=0b000110`

```
conj_matrix[:, 1] = [0, 1, 0, 0, 0, 0, 0, 0]
                         ↑conj1用

effective =         [0b111111, 0b000110, 0b111111, ..., 0b111111]

conj_results        [0b010101, 0b111111, 0b111111, ..., 0b111111]  (before)
              AND   [0b111111, 0b000110, 0b111111, ..., 0b111111]  (effective)
            ────────────────────────────────────────────────────
              =     [0b010101, 0b000110, 0b111111, ..., 0b111111]  (after)
                     ↑不变      ↑{1,2}
```

---

**迭代 p=2** — 谓词: NOT city=广州, `all_bm[2]=0b110111`

```
conj_matrix[:, 2] = [0, 1, 0, 0, 0, 0, 0, 0]
                         ↑conj1用

effective =         [0b111111, 0b110111, 0b111111, ..., 0b111111]

conj_results        [0b010101, 0b000110, 0b111111, ..., 0b111111]  (before)
              AND   [0b111111, 0b110111, 0b111111, ..., 0b111111]  (effective)
            ────────────────────────────────────────────────────
              =     [0b010101, 0b000110, 0b111111, ..., 0b111111]  (after)
                     ↑不变      ↑{1,2} 不变!

  逐位验证 conj 1:  0b000110 (女:1,2) & 0b110111 (NOT广州:0,1,2,4,5)
    bit 0: 0&1=0   item0 不是女
    bit 1: 1&1=1 ✓ item1 女+上海(非广州)
    bit 2: 1&1=1 ✓ item2 女+北京(非广州)
    bit 3: 0&0=0   item3 广州被排除, 但本来就不是女
    bit 4: 0&1=0   item4 不是女
    bit 5: 0&1=0   item5 不是女
    结果: 0b000110 = {item 1, 2}  (女性中无人在广州, 所以 NOT 广州未改变结果)
```

---

**迭代 p=3..15** — all_bm 均为 all_ones，不变，跳过。
...
```
conj_matrix[:, 16] = [1, 0, 0, 0, 0, 0, 0, 0]
                       ↑conj0用

effective =          [0b111001, 0b111111, 0b111111, ..., 0b111111]

conj_results         [0b010101, 0b000110, 0b111111, ..., 0b111111]  (before)
              AND    [0b111001, 0b111111, 0b111111, ..., 0b111111]  (effective)
            ─────────────────────────────────────────────────────
              =      [0b010001, 0b000110, 0b111111, ..., 0b111111]  (after)
                      ↑变了!     ↑不变
  ...
  逐位验证 conj 0:  0b010101 (北京:0,2,4) & 0b111001 (价格<100:0,3,4,5)
    ...
    结果: 0b010001 = {item 0, 4}  ← item2 因价格被淘汰
```

---

**迭代 p=17** — 谓词: price>=50, `all_bm[17]=0b100111`

```
conj_matrix[:, 17] = [0, 1, 0, 0, 0, 0, 0, 0]
                          ↑conj1用

effective =          [0b111111, 0b100111, 0b111111, ..., 0b111111]

conj_results         [0b010001, 0b000110, 0b111111, ..., 0b111111]  (before)
              AND    [0b111111, 0b100111, 0b111111, ..., 0b111111]  (effective)
            ─────────────────────────────────────────────────────
              =      [0b010001, 0b000110, 0b111111, ..., 0b111111]  (after)
                      ↑不变      ↑{1,2} 不变!

  逐位验证 conj 1:  0b000110 (女&非广州:1,2) & 0b100111 (价格≥50:0,1,2,5)
    bit 0: 0&1=0   item0 不在 conj1 候选中
    bit 1: 1&1=1 ✓ item1 128≥50
    bit 2: 1&1=1 ✓ item2 299≥50
    bit 3: 0&0=0
    bit 4: 0&0=0
    bit 5: 0&1=0   item5 不是女
    结果: 0b000110 = {item 1, 2}  (两位女性价格都≥50, 所以未变)
```
...
```
conj_results[0] = 0b010001 = {item 0, 4}    ← 北京 AND price<100
conj_results[1] = 0b000110 = {item 1, 2}    ← 女 AND NOT广州 AND price≥50
...
```
Step 1: [8] → [4]
  cr[0] = conj_results[0] | conj_results[1] = 0b010001 | 0b000110 = 0b010111
  cr[1] = conj_results[2] | conj_results[3] = 0b000000 | 0b000000 = 0b000000
  cr[2] = conj_results[4] | conj_results[5] = 0b000000 | 0b000000 = 0b000000
  cr[3] = conj_results[6] | conj_results[7] = 0b000000 | 0b000000 = 0b000000

Step 2: [4] → [2]
  cr[0] = 0b010111 | 0b000000 = 0b010111
  cr[1] = 0b000000 | 0b000000 = 0b000000

Step 3: [2] → [1]
  result = 0b010111 | 0b000000 = 0b010111
```

最终: `result & valid_mask = 0b010111 & 0b111111 = 0b010111`
...
word = 0b010111
  ctz(0b010111) = 0 → item 0 (北京, 男, 68)
  word &= word-1  → 0b010110
  ctz(0b010110) = 1 → item 1 (上海, 女, 128)
  word &= word-1  → 0b010100
  ctz(0b010100) = 2 → item 2 (北京, 女, 299)
  word &= word-1  → 0b010000
  ctz(0b010000) = 4 → item 4 (北京, 男, 15)
...
逐 item 核对两个 conjunction:

```
       │ conj0: city=北京 AND price<100 │ conj1: gender=女 AND NOT city=广州 AND price≥50 │ OR  │
───────┼────────────────────────────────┼────────────────────────────────────────────────┼─────┤
item 0 │ 北京 ✓  68<100 ✓  → match     │ 男 ✗                                           │ ✓   │
item 1 │ 上海 ✗                        │ 女 ✓  上海≠广州 ✓  128≥50 ✓  → match           │ ✓   │
item 2 │ 北京 ✓  299≥100 ✗             │ 女 ✓  北京≠广州 ✓  299≥50 ✓  → match           │ ✓   │
item 3 │ 广州 ✗                        │ 男 ✗                                           │ ✗   │
item 4 │ 北京 ✓  15<100 ✓  → match     │ 男 ✗                                           │ ✓   │
item 5 │ 上海 ✗                        │ 男 ✗                                           │ ✗   │
```
...
- **OR**: conj0 命中 {0,4}，conj1 命中 {1,2}，最终取并集 {0,1,2,4}
- **NOT**: city=广州 的 bitmap `0b001000` 经 XOR+mask 变为 `0b110111`，在 AND 中排除广州 item
- **数值范围**: `price<100` 和 `price>=50` 各自独立比较→打包，分别参与不同 conjunction
- **固定开销**: 尽管只用了 5 个有效谓词和 2 个 conjunction，forward() 仍执行全部 24 次 AND 迭代和 3 步 OR 规约——这就是固定计算量换可预测延迟的 trade-off
